...
function formularioEditarCandidata(capb)
{
    ...
    con.query("SELECT candidata.*, civil.nom_civ, categoria.nom_cat, municipio.nom_mun FROM candidata INNER JOIN civil ON candidata.fky_civ=civil.cod_civ INNER JOIN categoria ON candidata.fky_cat=categoria.cod_cat INNER JOIN municipio ON candidata.fky_mun=municipio.cod_mun WHERE cod_can='" + captionid + "'" , function (err, result, fields) 
    {
			        ...
			        for (i = 0; i < tam; i++) 
			        {
			            text = '<form autocomplete="off">'
			            	...
			            	text += '<div class="row">'
			            	    ...
			            	    text += '<div class="col-md-2">'
				            		text += '<div class="form-group label-floating">'
				            			text += '<label class="control-label">Fecha</label>'
				            			text += '<input type="text" class="form-control" id="fec_can" value="'+ result[i].fec_can +'" readonly>'
				            		text += '</div>'
				            	text += '</div>'
							text += '</div>'
...
function editarCandidata()
{
	...
	" fky_cat= '"+ categoriafk +"',"+" edad_can='"+ edad +"',"+" fky_mun ='"+ municipio +"',"+" dir_can ='"+ direccion +"',"+
	" tel_can ='"+ telefono +"',"+" email_can ='"+ email +"',"+" fec_can ='"+ fecha +"',"+" est_can ='"+ estado +"' WHERE cod_can = "+ id;
