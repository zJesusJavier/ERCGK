...
function selectCertamen() {
    con.query("SELECT * FROM certamen WHERE est_cer='A'", function (err, result, fields) {
        ...
        for (i = 0; i < tam; i++) {
            text += '<option value="' + result[i].cod_cer + '">' + result[i].des_cer + '</option>';
        }
        text += '</select>';
        document.getElementById("fky_cer").innerHTML = text;
    });
}
...
function selectPregunta1() {
    con.query("SELECT * FROM pregunta WHERE est_pre='A'", function (err, result, fields) {
        ...
        text += '</select>';
        document.getElementById("fky_pre_1").innerHTML = text;
    });
}
...
function selectPregunta2() {
    con.query("SELECT * FROM pregunta WHERE est_pre='A'", function (err, result, fields) {
        ...
        text += '</select>';
        document.getElementById("fky_pre_2").innerHTML = text;
    });
}